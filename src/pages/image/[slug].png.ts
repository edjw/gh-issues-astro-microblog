...
export async function get({ params }: APIContext) {
  ...
  const markup = html`<div class="bg-white flex flex-col w-full h-full">
    <div class="flex flex-col w-full h-4/5 p-10 justify-center">
      <div class="text-zinc-400 text-2xl mb-6">${date}</div>
      <div
    ...
    <div
      class="w-full h-1/5 border-t border-zinc-700/50 flex p-10 items-center justify-between text-2xl"
    >
      <div class="flex items-center">
        ...
        <span class="ml-3 text-zinc-400">edjohnsonwilliams.co.uk</span>
      </div>
      <div class="flex items-center">
        <img
        ...
        <div class="flex flex-col ml-4">
          <span class="text-zinc-400">Ed Johnson-Williams</span>
          <!-- <span class="text-blue-400">@kevinzunigacuel</span> -->
        </div>
      </div>
    </div>
  </div>`;
  const svg = await satori(markup, {
    ...
    fonts: [
      {
        name: "Raleway",
        data: Buffer.from(RalewayRegular),
        weight: 400,
      },
      {
        name: "Raleway",
        data: Buffer.from(RalewayBold),
        weight: 700,
      },
    ],
  });
